...
```bash
WORLD_FILE="$(rospack find gazebo_worlds)/worlds/uav_complex_120m.world" \
...
```text
gazebo_worlds/models/uav_complex_scene/materials/
```
...
```bash
python3 gazebo_worlds/worlds/generate_uav_complex_120m.py
```

生成结果固定为 `gazebo_worlds/worlds/uav_complex_120m.world`。
